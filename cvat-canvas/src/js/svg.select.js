...
* pointer events so that selection handles work with pen input
* (Apple Pencil). Fingers intentionally do not grab handles: on touch
* devices they pan/pinch the canvas instead (see pointerRouter.ts).
*/;
...
SelectHandler.prototype.drawPoints = function () {
    ...
    for (var i = 0, len = array.length; i < len; ++i) {
        ...
        // add css-classes and a pointerdown-event which fires our event for moving points
        var point = this.drawPoint(array[i][0], array[i][1])
                        .addClass(this.options.classPoints)
                        .addClass(this.options.classPoints + '_point')
                        .on('pointerdown', curriedEvent)
...
SelectHandler.prototype.selectRect = function (value) {
    ...
    // helperFunction to create a pointer-down function which triggers the event specified in `eventName`
