...
;(function () {
    ...
    ResizeHandler.prototype._extractPosition = function(event) {
        // Extract a position from a mouse/pointer/touch event.
        // Returns { x: .., y: .. }
        return {
    ...
    // one the gesture started with; synthetic events (dispatched by
    // cvat-canvas to force-stop a resize) and plain MouseEvents always pass
    ResizeHandler.prototype._ownsEvent = function (event) {
        return typeof event.pointerId !== 'number' ||
            this.pointerId === null ||
    ...
    ResizeHandler.prototype.resize = function (event) {
        ...
        this.pointerId = typeof startEvent.pointerId === 'number' ? startEvent.pointerId : null;

        var txPt = this._extractPosition(startEvent);
        this.parameters = {
        ...
        // When resizing started, we have to register events for pointer
        // movement and release; touch pointers are ignored: fingers pan and
        ...
        SVG.on(window, 'pointerup.resize', function (e) {
            if (_this._ownsEvent(e)) {
                _this.done();
            }
        });
        SVG.on(window, 'pointercancel.resize', function (e) {
            if (_this._ownsEvent(e)) {
                _this.done();
            }
        });
    ...
    // Is called on pointerup.
    // Removes the update-function from the pointermove event
    ResizeHandler.prototype.done = function () {
        this.lastUpdateCall = null;
        SVG.off(window, 'pointermove.resize');
        SVG.off(window, 'pointerup.resize');
        SVG.off(window, 'pointercancel.resize');
        this.pointerId = null;
